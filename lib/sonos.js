...
Sonos.prototype.currentTrack = function (callback) {
  ...
  return this.request(this.options.endpoints.transport, action, body, responseTag, function (err, data) {
    ...
    var trackposition = 0;
    if (data[0].Track.length > 0) {
      trackposition = parseInt(data[0].Track[0], 10)
    }
    ...
    if ((metadata) && (metadata[0].length > 0) && metadata[0] !== 'NOT_IMPLEMENTED') {
      ...
      return (new xml2js.Parser()).parseString(metadata, function (err, data) {
        ...
        track.uri = uri
        track.playlistposition = trackposition
 ...
 * @param  {Function} callback (err, playing)
 */
Sonos.prototype.playStream = function (uri, callback) {
  debug('Sonos.playStream(%j, %j)', uri, callback)
  var action
  var body
  var self = this
  var cb = (typeof uri === 'function' ? uri : callback) || function () {}
...
/**
 * Remove specific track in queue
 * @param  {Number}   position, first item in queue is 1
 * @param  {Function} callback (err, data)
 */
Sonos.prototype.removeTrack = function (position, callback) {
  debug('Sonos.removeTrack(%j)', callback)
  var action = '"urn:schemas-upnp-org:service:AVTransport:1#RemoveTrackFromQueue"'
  var body = '<u:RemoveTrackFromQueue xmlns:u="urn:schemas-upnp-org:service:AVTransport:1"><InstanceID>0</InstanceID><UpdateID>0</UpdateID><ObjectID>Q:0/' + position + '</ObjectID></u:RemoveTrackFromQueue>'
  return this.request(this.options.endpoints.transport, action, body, 'u:RemoveTrackFromQueueResponse', function (err, data) {
    if (err) return callback(err)
    if (data[0].$['xmlns:u'] === 'urn:schemas-upnp-org:service:AVTransport:1') {
      return callback(null, true)
    } else {
...
Sonos.prototype.setPlayMode = function (playmode, callback) {
  ...
  var mode = { NORMAL: true, REPEAT_ALL: true, SHUFFLE: true, SHUFFLE_NOREPEAT: true, SHUFFLE_REPEAT_ONE: true }[playmode.toUpperCase()]
...
Sonos.prototype.getPlayMode = function (callback) {
   ...
   var state = null
   return this.request(this.options.endpoints.transport, action, body, 'u:GetTransportSettingsResponse', function (err, data) {
     if (err) {
       callback(err)
       return
     }
